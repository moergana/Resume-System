...
## 基于 Vue + SpringBoot + LangChain/LangGraph 的简历分析系统
...
- 用户在注册账号时不仅需要这是用户名和密码，还同时需要绑定邮箱，邮箱用于在用户遗忘密码时验证身份并重置密码。
- 用户登录状态通过JWT验证。

#### 2、求职者用户的功能：

- 求职者用户能够在主页上传自己的简历，也可以访问保存所有职位的仓库。
- 选择特定的职位与自己上传过的简历进行匹配度分析（由LangGraph实现的AI Workflow驱动），最终可以获取到针对性的简历改进建议和求职建议。
- 为指定的上传过的简历推荐匹配度最高的几个职位（基于Embedding Model + Vector Database + 混合搜索）。
- 对分析结果历史记录进行带有多种过滤条件的分页查询（匹配度分析和推荐的结果都会保存到历史记录中）。
- 使用“求职小助手”功能，提供更多求职建议。该助手系统基于LangGraph + Agent + MCP，并配合有基于历史总结的上下文压缩功能。聊天历史上下文保存在PostgreSQL数据库中，使用前需要先执行工具包提供的初始化方法构建数据表。

#### 3、招聘者用户的功能：

- 招聘者用户能够在主页发布新的职位，也可以访问保存所有简历的仓库。
- 选择特定的简历与自己发布过的职位进行匹配度分析（由LangGraph实现的AI Workflow驱动），最终可以获取到简历与岗位的差异点分析报告。
- 为指定的发布过的职位推荐匹配度最高的几个简历（基于Embedding Model + Vector Database + 混合搜索）。
- 对分析结果历史记录进行带有多种过滤条件的分页查询（匹配度分析和推荐的结果都会保存到历史记录中）。
- 使用“求职小助手”功能，提供更多招聘建议。该助手系统基于LangGraph + Agent + MCP，并配合有基于历史总结的上下文压缩功能。聊天历史上下文保存在PostgreSQL数据库中，使用前需要先执行工具包提供的初始化方法构建数据表。

#### 4、简历与职位的管理（CRUD）：

- 用户有管理他们上传的简历或者发布的职位的功能。对于简历，用户可以上传、下载、删除的功能；对于职位，用户可以创建、查看、下载、删除。
- 简历和职位文件支持上传的格式有多种，例如PDF、DOCX、JPG等。
- 暂时没有手动输入创建简历的功能；职位可以选择手动输入，也可以选择上传文件，两种模式二选一，并且如果手动输入，下载功能将无效（因为创建过程中没有上传文件），而如果上传文件，那么手动输入框内的信息栏目在查看页面内显示为空（暂未构建AI分析结果写入对应栏目的功能）。

#### 5、表单的分页和筛选：

- 系统内所有表单都使用分页查询，并且配备有一些筛选条件设置框或按钮，实现带有过滤条件的分页查询。
- 带有过滤的分页查询由Java后端的Mybatis-plus构建动态SQL实现，分页和过滤的参数由前端传递（目前页面大小固定为10）。

#### 6、流量控制：

- 系统使用了Bucket4j的Redis实现版本以实现流量控制，针对请求IP地址进行限流，限流算法为令牌桶算法。
- 后续考虑引入更加专业的网关中间件做进一步优化。
