...
with mysql_engine.connect() as connection:
    result = connection.execute(
        text("""SELECT tb_jd.summary as jd_summary, tb_retrieve_resume.retrieve_result as retrieve_result
             FROM tb_retrieve_resume join tb_jd ON tb_retrieve_resume.jd_id = tb_jd.jd_id""")
    )
    ...
    for resume_record in resume_rows:
        test_case = LLMTestCase(
            input=resume_record.jd_summary,  # 输入是JD的摘要
...
def eval_retrieve_resume(test_cases: List[LLMTestCase],
                         name: str = "Retrieve Resume Evaluation",
                         criteria: str = default_retrieve_resume_criteria,
                         evaluate_steps: List[str] = default_retrieve_resume_evaluation_steps):
    """
    ...
    evaluate(
        ...
        metrics=[retrieve_resume_metric],
        async_config=AsyncConfig(max_concurrent=1, throttle_value=60)
    )
    
    
# ------------------ 开始评估测试 ------------------
eval_retrieve_resume(test_cases=test_cases)