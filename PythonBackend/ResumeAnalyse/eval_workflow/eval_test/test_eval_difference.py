...
with mysql_engine.connect() as connection:
    result = connection.execute(
        text("""SELECT tb_resume.summary as resume_summary, tb_jd.summary as jd_summary, tb_analysis_difference.analysis_result as difference_result
             From tb_analysis_difference join tb_resume on tb_analysis_difference.r_id = tb_resume.id
             join tb_jd on tb_analysis_difference.j_id = tb_jd.id""")
    )
    analysis_rows = result.fetchall()  # 获取所有差异分析的记录
...
def eval_difference(test_cases: List[LLMTestCase],
                    name: str = "Difference Evaluation",
                    criteria: str = difference_criteria,
                    evaluation_steps: List[str] = difference_evaluation_steps
                    ) -> float:
    """
    使用DeepEval评估LLM生成的差异内容的质量，输入原始内容和差异内容，输出评估得分。
    """
    # 定义评估总结的指标对象，基于G-Eval算法
    # 设计评估标准和步骤，使用OpenRouter AI作为评估模型，输入参数包括原始内容和总结内容
    difference_metric = GEval(
        name=name,
        criteria=criteria,
    ...
    evaluate(
        ...
        metrics=[difference_metric],
        async_config=AsyncConfig(max_concurrent=1, throttle_value=60)
    )
    

# ------------------ 开始评估测试 ------------------
eval_difference(test_cases=test_cases)