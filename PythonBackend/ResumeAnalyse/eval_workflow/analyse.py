...
def parse_difference_to_str(difference: Difference) -> str:
    """
    将Difference对象转换为字符串形式
    :param difference: Difference对象
    :return: 字符串形式的差异分析结果
    """
    result = f"""匹配度分数: {difference.match_score}
差异点分析: {difference.differences}"""
    return result
...
def parse_advice_to_str(advice: Advice) -> str:
    """
    将Advice对象转换为字符串形式
    :param advice: Advice对象
    :return: 字符串形式的建议结果
    """
    result = f"""匹配度分数: {advice.match_score}
简历改进建议: {advice.improvement_suggestions}
求职建议: {advice.job_hunting_tips}"""
    return result
